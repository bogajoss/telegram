...
async function inspect() {
  try {
    ...
    const userAttrs = await api('GET', `/databases/${DATABASE_ID}/collections/${USER_COLLECTION_ID}/attributes`);
    console.log(JSON.stringify(userAttrs, null, 2));

    console.log("\n--- Inspecting Posts Collection ---");
    const postAttrs = await api('GET', `/databases/${DATABASE_ID}/collections/${POST_COLLECTION_ID}/attributes`);
    console.log(JSON.stringify(postAttrs, null, 2));
  } catch (error) {
    console.error(error);
  }
}
